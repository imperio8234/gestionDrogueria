...
export const Search = ({ setData, getApi}) => {
    const [words, setWords] = useState([]);
     ...
     async function buscador(){
       try {
        const res = await fetch(`http://localhost:2000/api/v1/creditos/find/${words}`, {
            method:"GET",
            credentials:"include",
            headers:{
    ...
    return (
        ...
        <form action="" className="">
           ...
           <input onChange={(e) => setWords(e.target.value)} className="form-control form-control-sm buscador rounded-5" type="search" name="buscadorOptionsNav" id="buscadorOptionsNav" />
            <div onClick={buscador} className=" link-primary ms-1" type="button" value="buscar" ><FontAwesomeIcon icon={faMagnifyingGlass}/></div>
           </div>
          </form>
        </>
    )
